...
def bleu(gold, outs, generator, of='preds.txt'):
  preds = []
  refs = []
  for i in range(len(outs)):
    ...
    for j in range(bs):
      ...
      for k in ref[:,j]:
        k = int(k.data[0])
        if k != onmt.Constants.EOS:
          r.append(str(k))
      refs.append([r])
    for j in range(bs):
      scores = generator(hyp[:,j,:])
      pred = scores.max(1)[1]
      p = []
      for k in pred:
        k = int(k.data[0])
        if k != onmt.Constants.EOS:
          p.append(str(k))
      preds.append(p)
  score = bleu_score.corpus_bleu(refs,preds)
  print(score)
  '''
  with open(of,'w') as f:
    for p in preds:
      f.write(' '.join([dicts['tgt'].idxToLabel[int(k)] for k in p])+'\n')
    f.write("--------------refs----------------\n")
    for ref in refs:
      for r in ref:
        f.write(' '.join([dicts['tgt'].idxToLabel[int(k)] for k in r])+'\n')
  '''
  return score
...
def eval(model, criterion, data, epoch):
    ...
    model.train()
    eval_bleu = bleu(all_t, all_o, model.generator, opt.save_model+"_e"+str(epoch)+".out.txt")
    return total_loss / total_words, total_num_correct / total_words, eval_bleu
...
def trainModel(model, trainData, validData, dataset, optim):
    ...
    for epoch in range(opt.start_epoch, opt.epochs + 1):
        ...
        valid_loss, valid_acc, valid_bleu = eval(model, criterion, validData, epoch)
        valid_ppl = math.exp(min(valid_loss, 100))
        print('Validation perplexity: %g' % valid_ppl)
        print('Validation accuracy: %g' % (valid_acc*100))
        print('Validation BLEU: %g' % valid_bleu)
        ...
        if epoch == opt.start_epoch:
          save_cpt = True
          highest_acc = 100*valid_acc
          highest_bleu = valid_bleu
          lowest_ppl = valid_ppl
        if 100*valid_acc > highest_acc:
          highest_acc = 100*valid_acc
          save_cpt = True
        if valid_ppl < lowest_ppl:
          lowest_ppl = valid_ppl
          save_cpt = True
        if valid_bleu > highest_bleu:
          highest_bleu = valid_bleu
          save_cpt = True
      
        if save_cpt:
        '''
        cptname = '%s_acc_%.2f_ppl_%.2f_bleu_%.2f_e%d.pt' % (opt.save_model, 100*valid_acc, valid_ppl, 100*valid_bleu, epoch)
